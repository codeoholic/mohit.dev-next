...
const Books = ( props ) => {
    ...
    const page = allBooks.find((page) => page.slugAsParams === props.url )
    const MDXContent = useMDXComponent( page.body.code )
	return (

    	<div className="bg-slate-200 dark:bg-slate-800 w-screen h-full">
			<Head>
        		<title>Book | Mohit.dev</title>
      		</Head>
			<Header
				small={ true }
			/>
			<div className="container mx-auto px-5 lg:px-0 lg:max-w-4xl min-h-screen">
                <Link href="/books">
                    <div className="flex items-center gap-2.5 mt-10">
                        ...
                        <div className="text-zinc-800 dark:text-zinc-300 text-sm">Books</div>
                    </div>
                </Link>
...
export async function getStaticPaths( params ) {
    ...
    const response = allBooks.map((page) => ({
...
export async function getStaticProps({ params }) {
	...
	const books = allBooks
  	return { props: { books, url: params.url } }
